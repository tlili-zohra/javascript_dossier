...
function displayTimetable(timetable) {
  ...
  return sortedTimetable.map(function (ele) {
    return `Route : ${capitalize(ele.routeName)}
        Departs : ${ele.departureTime.toLocaleTimeString("fr-FR", {
          hour: "2-digit",
          minute: "2-digit",
          hour12: true,
...
function checkActiveBuses(timetable) {
  let currentTime = new Date(); // Get the current system time
  currentTime;
  let activeBuses = []; // To store active buses

  timetable.forEach(function (ele) {
    // Check if current time is between departure and arrival time
    if (currentTime >= ele.departureTime && currentTime <= ele.arrivalTime) {
      // Add bus details to activeBuses array
      activeBuses.push(`Route: ${ele.routeName} | Status: Active`);
    }
  });

  // After the loop, check if there are any active buses
  if (activeBuses.length) {
    activeBuses.forEach((bus) => console.log(bus)); // Log active buses
  } else {
    console.log("No active buses at the moment.");
  }

  return activeBuses; // Return the array of active buses
}

// Testing the function
console.log(checkActiveBuses(timetable));

function checkActiveBusesv2(timetable) {
  let currentTime = new Date();
  let result = timetable.filter(function (ele) {
    ele;
    return currentTime >= ele.departureTime && currentTime <= ele.arrivalTime;
  });
  return result;
}
console.log(checkActiveBusesv2(timetable));
